...
export const AgentAxiosAndTest = ({ request }) => AgentAxios({
    ...
    return runTestScript(request.testScript, { body: res.data, headers: [], status: 200 }).then((testDescriptor) => {
        return {
            response: res,
...
export const AgentAxiosCompare = ({ request }) => {
    return Promise.all([
        AgentAxios({
            method: request.method,
            url: request.endpoint,
    ...
    ]).then((res) => res.reduce((previousValue, currentValue, currentIndex) => {
        if (currentIndex === 0) {
