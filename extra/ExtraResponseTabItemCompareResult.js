...
const ExtraResponseTabItemCompareResult = ({ responses, requestaxios }) => {
    ...
    const columns = [
        {
            ...
            render(pathPair) {
                ...
                for (let i = 0; i < pathPair.leftUnmatchedPath.length; i++) {
                    leftArr.push(pathPair.leftUnmatchedPath[i].nodeName
                        ? pathPair.leftUnmatchedPath[i].nodeName
                        : pathPair.leftUnmatchedPath[i].index);
                }
                return _jsx("div", { children: leftArr.join('.') });
            },
        },
        {
            title: 'Right Path',
            ...
            render(pathPair) {
                ...
                for (let i = 0; i < pathPair.rightUnmatchedPath.length; i++) {
                    rightArr.push(pathPair.rightUnmatchedPath[i].nodeName
                        ? pathPair.rightUnmatchedPath[i].nodeName
                        : pathPair.rightUnmatchedPath[i].index);
                }
                return _jsx("div", { children: rightArr.join('.') });
            },
        },
        {
            title: 'Left Value',
    ...
    const [activeRadio, setActiveRadio] = useState('json');
    ...
    return (_jsx(Tabs, { style: { height: '100%' }, defaultActiveKey: 'compareResult', items: [
            {
                key: 'compareResult',
                label: 'Compare Result',
                children: (_jsxs(_Fragment, { children: [_jsx("div", { style: { textAlign: 'right', marginBottom: '10px' }, children: _jsx(Radio.Group, { size: 'small', options: optionsWithDisabled, onChange: (val) => {
                                    setActiveRadio(val.target.value);
                                }, value: activeRadio, optionType: 'button', buttonStyle: 'solid' }) }), _jsx("div", { className: 'react-diff-code-view', style: { height: '100%', display: activeRadio === 'json' ? 'block' : 'none' }, children: _jsxs("div", { id: 'MsgWithDiffJsonEditorWrapper', css: css `
                    overflow: hidden;
                  `, children: [_jsx("div", { ref: containerLeftRef, id: 'containerLeft' }), _jsx("div", { ref: containerRightRef, id: 'containerRight' })] }) }), _jsx("div", { style: { display: activeRadio === 'table' ? 'block' : 'none' }, children: _jsx(Table, { dataSource: dataSource, columns: columns }) })] })),
            },
        ], onChange: onChange }));
};
